...
function App() {
  ...
  const [titleReadabilityEffect, setTitleReadabilityEffect] = useState(DEFAULT_TITLE_READABILITY_EFFECT);
  ...
  const titleReadabilityEffectRef = useRef(titleReadabilityEffect);
  ...
  useEffect(()=>{
    ...
    setTitleReadabilityEffect(saved.titleStyle?.titleReadabilityEffect || DEFAULT_TITLE_READABILITY_EFFECT);
  },[]);
  ...
  const goToStep = (step) => {
    ...
    if (nextStep === 4) setTimeout(() => renderHTMLPoster(contentValuesRef.current, productTypeRef.current, titleFont, titleColor, textColorRef.current, currentBackgroundRef.current, slotImagesRef.current, titleReadabilityEffectRef.current), 60);
  };

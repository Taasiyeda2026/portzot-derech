...
function App() {
  ...
  const schoolLogoImageRef  = useRef(null);
  ...
  useEffect(()=>{
    ...
    schoolLogoImageRef.current = saved.schoolLogoImage || null;
  },[]);

  useEffect(() => {
    saveProject({ ...loadProject(), posterSize, productType, background: currentBackground, contentValues, fieldSettings, titleStyle: { fontFamily: titleFont, color: titleColor, textColor }, slotImages, schoolLogoImage: schoolLogoImageRef.current || null });
    if (wizardStep === 4) {
      // Debounce poster re-render so rapid keystrokes don't thrash the DOM
      clearTimeout(renderTimerRef.current);
      renderTimerRef.current = setTimeout(() => {
        renderHTMLPoster(contentValuesRef.current, productTypeRef.current, titleFont, titleColor, textColorRef.current, currentBackgroundRef.current, slotImagesRef.current, schoolLogoImageRef.current);
      }, 120);
    }
  ...
  const handleBackground = (path) => { currentBackgroundRef.current = path; setCurrentBackground(path); saveProject({ ...loadProject(), background: path }); if (wizardStep===4) renderHTMLPoster(contentValuesRef.current, productTypeRef.current, titleFont, titleColor, textColorRef.current, path, slotImagesRef.current, schoolLogoImageRef.current); };
  ...
  const handleExportPdf = () => {
    ...
    saveProject({
      ...
      schoolLogoImage: schoolLogoImageRef.current || null
    });
  ...
  const goToStep = (step) => {
    ...
    if (nextStep === 4) setTimeout(() => renderHTMLPoster(contentValuesRef.current, productTypeRef.current, titleFont, titleColor, textColorRef.current, currentBackgroundRef.current, slotImagesRef.current, schoolLogoImageRef.current), 60);
  };
  ...
  return h('div',{className:'app'}, h('div',{className:'step4-wrapper',style:{display:isStep4?'flex':'none'}}, h('div',{className:'step4-bar'}, h('div',{className:'step4-bar-start'}, h('button',{className:'step4-back-btn',onClick:()=>{if(productType==='physical'){setPhysicalSubStep(2);} goToStep(3);}},'‹ חזרה')), h('div',{className:'step4-bar-center'},productType==='physical'?h(PhysicalStepIndicator,{current:4}):h(StepIndicator,{current:4})), h('div',{className:'step4-bar-end'}, h('button',{className:'step4-export-btn',onClick:handleExportPdf},'ייצוא PDF'), h('button',{className:'step4-finish-btn',onClick:()=>goToStep(5)},'סיום ›'))),
    h('div',{className:'step4-body'}, h('div',{className:'html-poster-area'}, h('div',{id:'poster-html',style:{width:'794px',height:'1123px',minHeight:'1123px',position:'relative',overflow:'visible',direction:'rtl',background:'#fff',display:'flex',flexDirection:'column',boxShadow:'0 16px 56px rgba(94,39,80,.2), 0 4px 16px rgba(0,0,0,.08)',flexShrink:0}}, h('div',{id:'poster-bg',style:{position:'absolute',inset:0,zIndex:0}}), h('div',{id:'poster-inner',style:{position:'relative',zIndex:1,display:'flex',flexDirection:'column',height:'1123px',minHeight:'1123px'}}, h('div',{style:{height:'5px',background:'linear-gradient(90deg,#5E2750,#d61f8c,#9b40c0,#5E2750)',flexShrink:0}}), h('div',{style:{padding:'16px 32px 10px',textAlign:'center',position:'relative',flexShrink:0}}, h('div',{style:{position:'absolute',top:'10px',right:'20px'}},h('img',{src:'/poster-builder/assets/logoposter.svg',alt:'פורצות דרך',style:{height:'88px',width:'auto',objectFit:'contain',filter:'drop-shadow(0 0 4px rgba(255,255,255,0.95)) drop-shadow(0 0 10px rgba(255,255,255,0.75))'}})), h('div',{id:'ph-school-logo-wrap',style:{display:'none',position:'absolute',top:'14px',left:'16px',zIndex:20,maxWidth:'120px',maxHeight:'60px'}},h('img',{id:'ph-school-logo-img',alt:'לוגו בית הספר',crossOrigin:'anonymous',style:{display:'block',maxWidth:'120px',maxHeight:'60px',width:'auto',height:'auto',objectFit:'contain',filter:'drop-shadow(0 0 4px rgba(255,255,255,0.9))'}})), h('div',{id:'ph-name',style:{fontWeight:900,lineHeight:1.1,marginBottom:'8px',wordBreak:'break-word',overflowWrap:'anywhere'}}), h('div',{id:'ph-title-line',style:{width:'120px',maxWidth:'620px',height:'3px',background:'linear-gradient(90deg,#5E2750,#d61f8c)',borderRadius:'2px',margin:'0 auto 10px'}}), h('div',{id:'ph-desc',style:{fontSize:'15px',fontWeight:700,maxWidth:'500px',margin:'0 auto',lineHeight:1.45}}), h('div',{id:'ph-team',style:{background:'rgba(255,255,255,.96)',border:'1px solid rgba(94,39,80,.2)',borderRadius:'10px',padding:'6px 12px',width:'fit-content',maxWidth:'390px',margin:'7px auto 0'}},h('div',{id:'ph-names',style:{fontSize:'10px',fontWeight:400,lineHeight:1.65}}),h('div',{id:'ph-school',style:{fontSize:'9px',fontWeight:400,lineHeight:1.45,marginTop:'2px'}}))), h('div',{id:'ph-main',style:{flex:'1 1 auto',display:'flex',flexDirection:'column',justifyContent:'space-between',gap:'6px',padding:'0 24px 8px'}}, h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'7px',flexShrink:0}}, h('div',{className:'ph-card'},h('div',{className:'ph-cap'},'הבעיה שזיהינו'),h('div',{id:'ph-problem',className:'ph-body'}),h('div',{id:'ph-audience',className:'ph-sub'})),h('div',{className:'ph-card'},h('div',{className:'ph-cap'},'שאלת החקר'),h('div',{id:'ph-rq',className:'ph-body'}))), h('div',{style:{flexShrink:0}},h('div',{id:'ph-images-label',style:{fontSize:'7.5px',fontWeight:700,color:'#5E2750',textTransform:'uppercase',letterSpacing:'1.5px',textAlign:'center',marginBottom:'4px'}}),h('div',{id:'ph-images-2',style:{display:'none',gridTemplateColumns:'1fr 1fr',gap:'10px'}},h('div',{id:'ph-img-physical-0','data-ph-img':'0','data-layout':'physical',className:'ph-img-frame',style:{height:'220px'}}),h('div',{id:'ph-img-physical-1','data-ph-img':'1','data-layout':'physical',className:'ph-img-frame',style:{height:'220px'}})),h('div',{id:'ph-images-app',style:{display:'none',gridTemplateColumns:'1fr 1fr 1fr',gap:'7px'}},h('div',{id:'ph-img-app-0','data-ph-img':'0','data-layout':'app',className:'ph-img-frame ph-notch',style:{height:'270px'}}),h('div',{id:'ph-img-app-1','data-ph-img':'1','data-layout':'app',className:'ph-img-frame ph-notch',style:{height:'270px'}}),h('div',{id:'ph-img-app-2','data-ph-img':'2','data-layout':'app',className:'ph-img-frame ph-notch',style:{height:'270px'}})),h('div',{id:'ph-images-web',style:{display:'none',gridTemplateColumns:'1fr 1fr 1fr',gap:'7px'}},h('div',{id:'ph-img-web-0','data-ph-img':'0','data-layout':'website',className:'ph-img-frame',style:{height:'185px'}}),h('div',{id:'ph-img-web-1','data-ph-img':'1','data-layout':'website',className:'ph-img-frame',style:{height:'185px'}}),h('div',{id:'ph-img-web-2','data-ph-img':'2','data-layout':'website',className:'ph-img-frame',style:{height:'185px'}}))), h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px',flexShrink:0}},h('div',{className:'ph-card'},h('div',{className:'ph-cap'},'החקר שביצענו'),h('ul',{id:'ph-research',className:'ph-bullets'})),h('div',{className:'ph-card'},h('div',{className:'ph-cap'},'תובנות מהחקר'),h('div',{id:'ph-findings',className:'ph-body'})),h('div',{className:'ph-card'},h('div',{className:'ph-cap'},'דרישות הפתרון'),h('ul',{id:'ph-reqs',className:'ph-bullets'}))), h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'7px',flexShrink:0}},h('div',{className:'ph-card'},h('div',{id:'ph-solution-cap',className:'ph-cap'}),h('div',{id:'ph-solution',className:'ph-body'})),h('div',{className:'ph-card'},h('div',{id:'ph-usage-cap',className:'ph-cap'}),h('ul',{id:'ph-usage',className:'ph-bullets'}))), h('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px',flexShrink:0}},h('div',{className:'ph-card ph-card-accent'},h('div',{className:'ph-cap'},'המשוב שקיבלנו'),h('div',{id:'ph-feedback',className:'ph-body'})),h('div',{className:'ph-card ph-card-accent'},h('div',{className:'ph-cap'},'מה שיפרנו'),h('div',{id:'ph-improved',className:'ph-body'})),h('div',{className:'ph-card ph-card-accent'},h('div',{className:'ph-cap'},'הערך המרכזי'),h('div',{id:'ph-value',className:'ph-body'})))), h('div',{id:'ph-footer',style:{background:'linear-gradient(135deg,#4a1f40,#5E2750,#9b40c0)',padding:'8px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}},h('div',{id:'ph-slogan',style:{fontStyle:'italic',fontSize:'13px',color:'rgba(255,255,255,.95)'}}),h('div',{style:{fontSize:'9.5px',color:'rgba(255,255,255,.6)',fontWeight:700,letterSpacing:'1.2px'}},'פורצות דרך ✦ 2026')))))))))),
